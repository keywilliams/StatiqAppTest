...
AsyncInit();Filters.searchKey.on('keyup',function(e){if(e.key==='Enter'||e.keyCode===13){TriggerSearch();}});$('#contentTypes>button').on("click",function(e){let target=$(e.target);if(target.hasClass("active"))
...
return false;if(filters.audio!=""&&filters.audio!=undefined&&!elem.Dubs?.filter((e)=>e==filters.audio).length)
return false;if(filters.subtitle!=""&&filters.subtitle!=undefined&&!elem.Subs?.filter((e)=>e==filters.subtitle).length)
...
const UpdateFiltersOptions=()=>{switch(Filters.activeFilter.find(".active").data().value){case ActiveContentTypeOption.All:Filters.category.empty();Filters.category.append(RenderSelectOptions(jsonData.IndexView.Categories.map((x)=>x.Description).sort((a,b)=>a.localeCompare(b))));Filters.genre.empty();Filters.genre.append(RenderSelectOptions(jsonData.IndexView.Genres.map((x)=>x.Description).sort((a,b)=>a.localeCompare(b))));Filters.audio.empty();Filters.audio.append(RenderSelectOptions(jsonData.IndexView.Languages.map((x)=>x.Description).sort((a,b)=>a.localeCompare(b))));Filters.subtitle.empty();Filters.subtitle.append(RenderSelectOptions(jsonData.IndexView.Subtitles.map((x)=>x.Description).sort((a,b)=>a.localeCompare(b))));break;case ActiveContentTypeOption.Audio:Filters.category.empty();Filters.category.append(RenderSelectOptions([...new Set(jsonData.IndexView.Audios.flatMap(x=>x.Categories))].sort((a,b)=>a.localeCompare(b))));Filters.genre.empty();Filters.genre.append(RenderSelectOptions([...new Set(jsonData.IndexView.Audios.flatMap(x=>x.Genres))].sort((a,b)=>a.localeCompare(b))));break;case ActiveContentTypeOption.TV:Filters.category.empty();Filters.category.append(RenderSelectOptions([...new Set(jsonData.IndexView.Tvs.flatMap(x=>x.Categories))].sort((a,b)=>a.localeCompare(b))));Filters.genre.empty();Filters.genre.append(RenderSelectOptions([...new Set(jsonData.IndexView.Tvs.flatMap(x=>x.Genres))].sort((a,b)=>a.localeCompare(b))));Filters.audio.empty();Filters.audio.append(RenderSelectOptions([...new Set(jsonData.IndexView.Tvs.flatMap(x=>x.Dubs))].sort((a,b)=>a.localeCompare(b))));Filters.subtitle.empty();Filters.subtitle.append(RenderSelectOptions([...new Set(jsonData.IndexView.Tvs.flatMap(x=>x.Subs))].sort((a,b)=>a.localeCompare(b))));break;case ActiveContentTypeOption.Movies:Filters.category.empty();Filters.category.append(RenderSelectOptions([...new Set(jsonData.IndexView.Movies.flatMap(x=>x.Categories))].sort((a,b)=>a.localeCompare(b))));Filters.genre.empty();Filters.genre.append(RenderSelectOptions([...new Set(jsonData.IndexView.Movies.flatMap(x=>x.Genres))].sort((a,b)=>a.localeCompare(b))));Filters.audio.empty();Filters.audio.append(RenderSelectOptions([...new Set(jsonData.IndexView.Movies.flatMap(x=>x.Dubs))].sort((a,b)=>a.localeCompare(b))));Filters.subtitle.empty();Filters.subtitle.append(RenderSelectOptions([...new Set(jsonData.IndexView.Movies.flatMap(x=>x.Subs))].sort((a,b)=>a.localeCompare(b))));break;default:break;}}
...
if(values.activeFilter!=""){Filters.activeFilter.children().removeClass('active');Filters.activeFilter.find($(`[data-value="${values.activeFilter}"]`)).addClass('active')}
...
const RenderResultCount=(num)=>{return`<p class="search-results__title hide-tablet"> Search Results <span>(${num})</span></p>`};const RenderMobileFilters=()=>{return`<div class="d-flex d-md-none justify-content-between align-items-center mb-4">
            ...
            </div>`;};const RenderResultCards=(items)=>{let html=`<div class="row ${items?.length ? 'row-cols-lg-5' : ''}">`;if(items.length){items.forEach((elem)=>html+=RenderCard(elem));}
...
if(item.hasOwnProperty("AllMovies")){return"icon-movie";}};const RenderSelectOptions=(items)=>{let html='<option value="-1"></option>';items&&items.forEach((elem)=>html+=`<option value="${elem}">${elem}</option>`);return html;}
...
let activeTab=$("#onYourFlightModal-tabs").find($('.active'))[0];let values;if(activeTab.id=='destination-tab'){values=GetModalDestinationTabValues();if(values.DateDeparture==""||values.FlightNumber==""||values.From==""||values.To==""){if(values.DateDeparture!=""||values.FlightNumber!=""||values.From!=""||values.To!=""){ShowErrorMessage();}
else{RemoveErrorMessage();CloseModalWithoutData();}
return;}
RemoveErrorMessage();Filters.flightDate.val(values.DateDeparture);Filters.flightNumber.val(values.FlightNumber);Filters.flightFrom.val(values.From);Filters.flightTo.val(values.To);ResetModalFlightTabValues();}else{values=GetModalFlightTabValues();if(values.DateDeparture==""||values.FlightNumber==""){if(values.DateDeparture!=""||values.FlightNumber!=""){ShowErrorMessage();}
else{RemoveErrorMessage();CloseModalWithoutData();}
return;}
...
RemoveAllErrorMessages();$("#contentFilters").collapse('show');Filters.flightDate.val(values.DateDeparture);Filters.flightNumber.val(values.FlightNumber);Filters.flightFrom.val(values.From);Filters.flightTo.val(values.To);TriggerSearch();$('#onYourFlightModal').modal('hide');}
